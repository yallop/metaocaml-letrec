...
type locus_t = int
let new_locus = let counter = ref 0 in fun () -> (incr counter; !counter)

effect MakeSlot : locus_t -> ('a ref code * 'a code)
effect SetSlot : (locus_t * 'a ref code * 'a code) -> unit
...
let genletrec : type a b. locus_t -> (a code -> a code) -> a code
  = fun locus f ->
      let lhs, use = perform (MakeSlot locus) in
      let () = perform (SetSlot (locus, lhs, f use)) in
      use
